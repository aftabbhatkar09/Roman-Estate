...
export default function PropertyGallery({
  ...
  return (
    <div className="max-w-[1440px] mx-auto px-3 sm:px-4 md:px-6 lg:px-8 mt-3 sm:mt-4">
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-3 sm:gap-4 h-[45vh] sm:h-[55vh] md:h-[65vh] lg:h-[70vh]">
        {/* Main Large Image */}
        <div className="lg:col-span-8 relative rounded-[1rem] sm:rounded-[1.5rem] md:rounded-[2rem] overflow-hidden group min-h-0">
          <ImageWithFallback
        ...
        <div className="hidden lg:flex lg:col-span-4 flex-col h-full bg-gray-50/80 p-4 sm:p-5 rounded-[1.5rem] md:rounded-[2rem] border border-gray-100/90 overflow-hidden">
          <div className="flex items-center justify-between pb-3.5 px-1 border-b border-gray-200/60 mb-4 shrink-0">
            <span className="text-xs font-black uppercase tracking-[0.2em] text-brand-dark flex items-center gap-2">
              <Images className="w-4 h-4 text-brand-primary" /> Photo Gallery ({allImages.length})
            </span>
      ...
      <div className="flex lg:hidden gap-2 mt-3 overflow-x-auto pb-2 custom-scrollbar">
        {allImages.map((img, i) => (
          ...
          >
            <ImageWithFallback src={img} alt="" fill sizes="100px" className="object-cover" />
          </button>
        ))}
      </div>
        ...
        <div className="fixed inset-0 z-[100] bg-brand-dark flex flex-col">
          <div className="flex items-center justify-between px-4 sm:px-6 md:px-8 py-4 sm:py-6">
            <div className="flex flex-col min-w-0">
              <span className="text-white font-black text-sm sm:text-base md:text-lg tracking-tight truncate max-w-[200px] sm:max-w-none">{title}</span>
              <span className="gradient-text text-[10px] sm:text-xs font-bold uppercase tracking-widest">{activeIdx + 1} of {allImages.length} Photos</span>
            </div>
            <button 
          ...
          <div className="flex-1 relative flex items-center justify-center px-2 sm:px-4 md:px-8 lg:px-20">
            ...
            <button 
              onClick={prev} 
              className="absolute left-1 sm:left-2 md:left-4 lg:left-8 w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 lg:w-16 lg:h-16 rounded-full bg-white/5 hover:bg-gradient-to-br hover:from-brand-primary hover:to-brand-accent flex items-center justify-center text-white transition-all border border-white/10"
              aria-label="Previous image"
            >
            ...
            <button 
              onClick={next} 
              className="absolute right-1 sm:right-2 md:right-4 lg:right-8 w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 lg:w-16 lg:h-16 rounded-full bg-white/5 hover:bg-gradient-to-br hover:from-brand-primary hover:to-brand-accent flex items-center justify-center text-white transition-all border border-white/10"
              aria-label="Next image"
            >
              ...
              {allImages.map((img, i) => (
                <button
                  ...
                  className={`relative w-16 h-10 sm:w-20 sm:h-14 md:w-24 md:h-16 rounded-lg sm:rounded-xl overflow-hidden transition-all duration-300 ${
                    i === activeIdx ? 'ring-2 ring-brand-primary scale-105 sm:scale-110 shadow-2xl' : 'opacity-40 hover:opacity-100'
                  }`}
                >
