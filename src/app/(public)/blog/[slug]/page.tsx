...
export default async function BlogDetailPage({ params }: BlogPageProps) {
  ...
  return (
    <article className="bg-white min-h-screen pb-24">
      {/* Article Header/Hero */}
      <div className="relative min-h-[480px] sm:min-h-[560px] md:min-h-[640px] w-full overflow-hidden bg-brand-dark flex flex-col justify-end pt-28 sm:pt-32 pb-8 sm:pb-12 md:pb-16">
        <ImageWithFallback
          src={blog.image}
          alt={blog.title}
          fill
          ...
          priority
        />
        <div className="absolute inset-0 bg-gradient-to-t from-brand-dark via-brand-dark/45 to-brand-dark/20" />
        ...
        <div className="relative z-10 w-full px-4 sm:px-6 md:px-12 lg:px-16">
          <div className="max-w-5xl mx-auto space-y-4 sm:space-y-6 md:space-y-8">
            ...
            >
              <ArrowLeft className="w-3 h-3 sm:w-4 sm:h-4 text-brand-primary shrink-0" /> Journal Archive
            </Link>
            ...
            <div className="space-y-3 sm:space-y-4 md:space-y-6">
              <div className="flex flex-wrap gap-1.5 sm:gap-2">
                {blog.tags?.map((tag: string) => (
                  <span
                    key={tag}
                    className="bg-gradient-to-r from-brand-primary to-brand-accent text-white px-2.5 sm:px-4 py-1 rounded-full text-[8px] sm:text-[10px] font-black uppercase tracking-widest shadow-lg shadow-brand-primary/30"
                  >
                    {tag}
                  </span>
                ))}
              ...
              <div className="flex flex-wrap items-center gap-4 sm:gap-6 md:gap-8 pt-2 sm:pt-4">
                <div className="flex items-center gap-3 sm:gap-4">
                  <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl bg-gradient-to-br from-brand-primary to-brand-accent flex items-center justify-center font-black text-white text-sm sm:text-lg shadow-xl shrink-0">
                    {blog.author.charAt(0)}
                  </div>
                  <div>
                    <p className="text-[8px] sm:text-[10px] text-gray-400 font-black uppercase tracking-[0.2em]">Author</p>
                    <p className="text-white font-black text-sm sm:text-base">{blog.author}</p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-3 sm:gap-4">
                  ...
                  <div>
                    <p className="text-[8px] sm:text-[10px] text-gray-400 font-black uppercase tracking-[0.2em]">Published</p>
      ...
      <div className="max-w-7xl mx-auto px-4 mt-8 sm:mt-12 md:mt-16 lg:mt-24 flex flex-col lg:flex-row gap-8 sm:gap-10 md:gap-12 lg:gap-20">
        <div className="lg:w-2/3">
          ...
          <div className="mt-12 sm:mt-16 md:mt-20 lg:mt-24 pt-6 sm:pt-8 md:pt-10 lg:pt-12 border-t border-gray-100 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 sm:gap-6 md:gap-8">
            <div className="space-y-1">
              <h4 className="text-base sm:text-lg md:text-xl font-black text-brand-dark tracking-tight">Share this perspective</h4>
              <p className="text-xs sm:text-sm text-gray-400 font-medium">Circulate this insight within your professional network.</p>
            </div>
            <div className="flex items-center gap-2 sm:gap-3 md:gap-4">
              {['Facebook', 'Twitter', 'LinkedIn'].map((social, i) => (
                <button
                  key={i}
                  className="px-3 sm:px-4 md:px-5 lg:px-6 py-2 sm:py-2.5 md:py-3 rounded-xl sm:rounded-2xl border border-gray-100 font-black text-[9px] sm:text-[10px] md:text-xs uppercase tracking-widest text-brand-dark hover:bg-gradient-to-br hover:from-brand-primary hover:to-brand-accent hover:text-white hover:border-transparent transition-all duration-300"
                >
                  {social}
                </button>
              ))}
        ...
        <aside className="lg:w-1/3 space-y-8 sm:space-y-10 md:space-y-12">
          <div className="bg-gray-50 p-6 sm:p-8 md:p-10 rounded-[1.5rem] sm:rounded-[2rem] md:rounded-[3rem] border border-gray-100 space-y-5 sm:space-y-6 md:space-y-8">
            <div className="flex items-center gap-2 sm:gap-3">
              <Tag className="w-4 h-4 sm:w-5 sm:h-5 text-brand-primary shrink-0" />
              <h4 className="text-base sm:text-lg md:text-xl font-black text-brand-dark tracking-tight">Curated Topics</h4>
            </div>
          ...
          <div className="bg-gradient-to-br from-brand-dark to-brand-dark-light p-6 sm:p-8 md:p-10 lg:p-12 rounded-[1.5rem] sm:rounded-[2rem] md:rounded-[3rem] lg:rounded-[3.5rem] text-white shadow-2xl relative overflow-hidden group">
          ...
          {/* Featured Listing Ad */}
          <div className="relative aspect-[4/5] max-h-[500px] rounded-[2rem] sm:rounded-[2.5rem] md:rounded-[3.5rem] overflow-hidden group">
             <Image 
             ...
             <div className="absolute bottom-0 left-0 w-full p-6 sm:p-8 md:p-10 space-y-3 sm:space-y-4">
                <span className="text-brand-primary-light text-[8px] sm:text-[10px] font-black uppercase tracking-[0.3em]">Featured Offering</span>
                <h5 className="text-lg sm:text-xl md:text-2xl font-black text-white tracking-tight">Penthouse at Worli Skyline</h5>
                <Link href="/properties" className="text-white font-bold text-[11px] sm:text-xs flex items-center gap-2 group/link">
                  View Masterpiece <ArrowRight className="w-3 h-3 sm:w-4 sm:h-4 group-hover/link:translate-x-1 transition-transform" />
                </Link>
             </div>
          </div>
        </aside>
