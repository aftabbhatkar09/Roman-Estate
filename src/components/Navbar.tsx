...
export default function Navbar() {
  ...
  return (
    <nav
      className={`fixed bg-white top-0 left-0 right-0 z-50 transition-all duration-500 py-3 ${scrolled
    ...
    >
        ...
        <div className="flex justify-between items-center">
          ...
          <Link href="/" className="flex items-center gap-2 group">
            <div className="flex items-center justify-between gap-2 leading-none">
              <Image src="/images/logo3.png" alt="Roman Estate Logo" height={60} width={60} />
              <div className="flex flex-col items-center justify-center font-serif text-black uppercase select-none mt-1">
                <span className="text-[10px] sm:text-[11px] tracking-[0.5em] ml-[0.5em] mb-0.5 font-medium">The</span>
                <span className="text-2xl sm:text-3xl tracking-widest font-normal leading-none">Roman</span>
                <div className="flex items-center justify-center gap-2 mt-1.5 w-full">
                  <div className="h-[1px] w-5 sm:w-8 bg-[#C2A365]"></div>
                  <span className="text-[9px] sm:text-[10px] tracking-[0.5em] ml-[0.5em] font-medium text-gray-800">Estate</span>
                  <div className="h-[1px] w-5 sm:w-8 bg-[#C2A365]"></div>
                </div>
              </div>
            </div>
            ...
            {navLinks.map((link) => {
              ...
              return (
                <Link
                  key={link.name}
                  href={link.href}
                  className={`relative text-md font-semibold transition-all duration-300 group ${isActive
                    ? "text-brand-primary"
      ...
      {/* Mobile Menu Overlay */}
      <div
        className={`fixed inset-0 bg-brand-dark/20 backdrop-blur-sm z-40 transition-opacity duration-300 md:hidden ${isOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"
          }`}
      ...
      {/* Mobile Menu */}
      <div
        className={`fixed top-0 right-0 bottom-0 w-[80vw] max-w-sm bg-white z-50 shadow-2xl transition-transform duration-500 ease-out md:hidden ${isOpen ? "translate-x-0" : "translate-x-full"
          }`}
      >
            ...
            {navLinks.map((link, i) => {
              ...
              return (
                <Link
                  ...
                  href={link.href}
                  onClick={() => setIsOpen(false)}
                  className={`flex items-center py-3.5 text-lg font-bold transition-all duration-300 ${isActive
                    ? "text-brand-primary"
